...
```{r wd}
setwd("C:/Users/jomely/Desktop/ikiam_2020/bioinformatica/examen_final/")
```
...
## creamos la función long_df que pide como parametro un data frame y pasamos de wider a long con la funcion gather 
library(tidyr)
library(dplyr)
long_df <- function(df) {
  df$dataset <- as.factor(df$dataset)
  df_1 <- gather(df, gen, expresion_level, GATA3, PTEN, XBP1, ESR1, MUC1, FN1, GAPDH, factor_key=TRUE)
  df_2 <- select(df_1, -bcr_patient_barcode)
  return (df_2)
}
...
## seleccionamos los datos que se van a realizar la tabla, en este caso se eligieron pacientes que presentaban cancer de ovario y se analizo los niveles de expresion de los siete genes. 

library(sjPlot)
library(sjmisc)
datos <- df_long %>% filter(dataset == "OV") %>% select(2:3) 
cancer_ovarios <- datos %>% group_by(gen) %>% mutate(row = row_number()) %>%
  tidyr::pivot_wider(names_from = gen, values_from = expresion_level) %>%
  select(-row)
descr(cancer_ovarios) %>% tab_df(title = "Estadistica descriptiva del cancer de ovario y su nivel el nivel de expresion", file = "results/mRNA_expr_summary.doc") 
...
## En la tabla creada presenta un resumen de algunas valores de estadistica descriptiva, se puede observar que hubieron 561 pacientes. Ademas, en la media se puede observar que existieron valores de expresion negativos. Finalmente el gen GAPDH y FN1 presentaron los valores de sd mas bajo y alto respectivamente. 
```
...
tcga_boxplots <- function(dataset) {
  return(ggplot (dataset, aes(x = gen, y = expresion_level, col = gen)) + labs(x="Gen", y = "Nivel de expresion")
         + geom_boxplot() + geom_jitter(size=0.2))
}
...
## creamos una lista con el nombre dataset1 y filtramos los 6 tipos de cancer  
dataset1 <- list(
...
## creamos la lista list_boxplots que contendra todos los graficos de los niveles de expresion de los diferentes tipos de cancer 
list_boxplots = lapply(dataset1, function(x) {ggplot(x, aes(dataset, expresion_level, col = dataset)) +
...
## el paso 1 fue crear datas frames con los 6 tipos de cancer  
BRCA <- filter(df_long, dataset == "BRCA")
OV <- filter(df_long, dataset == "OV")
LUSC <- filter(df_long, dataset == "LUSC")
KIPAN <- filter(df_long, dataset == "KIPAN")
...
## cambiamos la lista de long a wider de cada uno de los tipos de cancer 
plots_cancer <- list (  
...
## generamos una función denominada reg_gen_expression que pide como parametro uno de los 7 genes de estudio para crear combinaciones entre pares de genes, obteniendo por gen 6 graficos. 
reg_gen_expression <- function (gen) {
## colocamos la condicional if y else if para graficar unicamente los genes de entrada 
  if (gen == "GATA3"){
    total <- list (
      ## se utilizo la función apply para aplicar los 6 graficos a los 6 tipos de cancer que hay 
...
```{r list_regplots}
# Escriba el código para generar la lista. 
## en este caso se selecciono el gen PTEN, en base al gen seleccionada se realizaran combinaciones para la obtencion de los 6 graficos correspondientes por cada tipo de cancer 
plots <- reg_gen_expression("PTEN") 
```
...
plots[[5]][[2]] ## la lista graficos cancer contiene las combinaciones del gen "PTEN"
...
```{tex}
# boxplot3.png se puede observar los niveles de expresion de todos valores de las personas con cancer de pulmon, se puede observar que la mediana es alrededor de -1 y que el 50% de los valores principales (Q1-Q3) estan entre -2 y 0. 

# regresion11.pdf se puede observar a los genes en el eje x a PTEN y en el eje y a FN1. El grafico es de cancer de ovarios y la linea de la regresion lineal indica que no existe una relacion marcada entre la expresion de ambos genes. 
